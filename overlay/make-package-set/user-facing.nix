...
let
    ...
    then
      ...
      then
        ...
        # don't use explicit "latest" attribute!  See oxalica README for more details.
          ...
          buildPackages.rust-bin.nightly.${args.rustVersion}.${rustProfile}.override toolchainArgs
      else
        if (
          (rustChannel != null &&
        ...
        then
          ...
          # rustChannel is actually a rustVersion.  Treat argument as legacy.
            buildPackages.rust-bin.stable.${args.rustChannel}.${rustProfile}.override toolchainArgs
        else
          let
            rustChannel' = if rustChannel != null then rustChannel else "stable";
          in
          buildPackages.rust-bin.${rustChannel'}.${args.rustVersion}.${rustProfile}.override toolchainArgs;
  ...
  # target.
in
rustBuilder.makePackageSetInternal (extraArgs // {
  inherit packageFun ignoreLockHash workspaceSrc target targetInfo cargoConfig registries;
  rustToolchain = rustToolchain';
  packageOverrides = packageOverrides' pkgs;
  buildRustPackages = buildPackages.rustBuilder.makePackageSetInternal (extraArgs // {
    inherit packageFun ignoreLockHash workspaceSrc targetInfo cargoConfig registries;
    rustToolchain = rustToolchain';
    target = null;
